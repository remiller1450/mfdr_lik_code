...
for (i in 1:length(nn)){
  ...
  for(q in 1:5){
    ...
    X.A <- genData(n, t, beta = c(rep(5/sqrt(n), t/2), rep(-5/sqrt(n), t/2)), J0=4, rho=0, family = "survival")
    X.C <- genData(n, p-t, rho=0.8, beta=0, corr="auto")
    yy <- Surv(X.A$y, rep(1, length(X.A$y)))
    X <- cbind(X.A$X, X.C$X)
    ### Fit Cox Model
    fit.lam <- ncvsurv(X, yy, returnX = TRUE, penalty = 'MCP', nlambda = 3)
    lmax <- c(lmax, max(fit.lam$lambda))
  }
  lseq <- seq(1.10*max(lmax),0.05*max(lmax), length.out = 200)
  ...
  save(result, result.lasso, nn, file = "sim1_cox_cor.RData")
}
